...
final class StatementsActivity extends EventDependentViewDomainActivity implements
    ...
    public Node buildUi() {
        ...
        EntityButtonSelector<Entity> moneyAccountSelector = new EntityButtonSelector<>( // language=JSON5
            "{class: 'MoneyAccount', alias: 'ma', columns: [{expression: '[icon,name,currency.icon]'}], where: 'type.internal', orderBy: 'closed,type desc,exists(select MoneyFlow where toMoneyAccount=ma and positiveAmounts) ? 0 : 1,id desc'}",
